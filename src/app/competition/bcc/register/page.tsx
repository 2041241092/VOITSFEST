...
export default function BccRegisterPage() {
  ...
  const [cmsPricing, setCmsPricing] = useState<EventPricing>(DEFAULT_PRICING_TIERS.bcc);

  useEffect(() => {
    async function loadPricing() {
      const tiers = await fetchPricingTiers();
      if (tiers.bcc) {
        setCmsPricing(tiers.bcc);
      }
    }
    loadPricing();
  }, []);
        ...
        <form onSubmit={handleSubmit} className="space-y-12 max-w-4xl mx-auto">
          ...
          <div className="glass-card rounded-2xl p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                <label className="font-poppins font-medium text-sm text-on-surface-variant uppercase tracking-wider block">Upload File Proposal BCC *</label>
                <p className="font-poppins text-xs text-on-surface-variant mb-2">Format Nama File: ProposalBCC_NamaTim_NamaKetuaTim. PDF (max. 1 file)</p>
                <div className="relative w-full">
